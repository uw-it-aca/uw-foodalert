<template>
  <b-container class="px-4 pt-3">
    <p>
      <strong>Thanks for Sharing! Here's what's next:</strong>
    </p>
    <p
      v-if="event != ''"
      style="color: green"
    >You are currently updating your event: "{{this.event}}"</p>
    <p
      v-if="event === ''"
      style="color: red"
    >You must be currently hosting an event to submit an update</p>
    ...
    <div v-if="v.form.text.$error">
      ...
      <p
        v-if="!v.form.text.maxLength"
        class="hasError"
      >Your update message must be shorter than 100 characters</p>
    </div>
    <b-link
      type="submit"
      :disabled="v.form.$invalid || event === ''"
      @click="$emit('submitRequest')"
      class="w-100 mb-3 py-2 btn btn-secondary btn-md"
    >Send Update</b-link>
    ...
    >
      <p class="h5 pb-3">Are you sure you'd like to notify student's there's
        <strong>no food left</strong>?
      </p>

      <blockquote
        class="border rounded mb-0 mx-3 p-3 text-left bg-light"
      >Update: No Food left! The event: “{{this.event}}” has ended and is no longer serving food.</blockquote>

      <template slot="modal-footer">
        <b-link
          class="btn btn-lg btn-primary mx-auto mb-2"
          :disabled="event === ''"
          @click="$emit('submitEnd')"
        >Send 'No Food Left'</b-link>
