...
export default {
    ...
    buildRequest() {
      var data = {
        ...
        time: {
          created: new Date(),
          ended: new Date(
            new Date().toString().substring(0, 16) + this.state.endTime + ":00"
          )
        },
        food: {
      ...
      axios
        .post("/notification/", data, { headers: headers })
        .then(
          function(response) {
  ...
  computed: {
    ...
    previewText: function() {
      ...
      var text = {
        heading: "An event has just been posted! Here are the details...",
